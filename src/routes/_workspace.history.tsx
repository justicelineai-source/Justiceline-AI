...
import { loadDrafts, saveDrafts, type DraftRecord } from "@/lib/draft-store";
...
export function HistoryPage() {
  ...
  const [draftsList, setDraftsList] = useState<DraftRecord[]>([]);
...
const handleFilterChange = (filter: FilterType) => {
  ...
  const handleDeleteItem = (item: UnifiedHistoryItem, e: React.MouseEvent) => {
    ...
    if (item.type === "chat") {
      ...
      const updated = draftsList.filter((d) => d.id !== item.id);
  ...
  const handleOpenItem = (item: UnifiedHistoryItem) => {
    if (item.type === "chat") {
      ...
      // Navigates directly to the legal drafting screen
      navigate({ to: "/draft" });
    }
  };
  ...
  const historyItems: UnifiedHistoryItem[] = useMemo(() => {
    ...
    const drafts: UnifiedHistoryItem[] = draftsList.map((d) => ({
      id: d.id,
      title: d.title || `${d.draftType} Draft`,
      type: "draft",
      modeName: d.draftType,
      timestamp: d.timestamp,
      route: "/draft",
    }));
 
    return [...chats, ...drafts].sort((a, b) => b.timestamp - a.timestamp);
